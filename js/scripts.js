...
// getGroundTracks({
//     tle: ISS_TLE,
//     startTimeMS: secondsSinceEpoch = Math.round(Date.now() / 1000),
//     stepMS: 1000,
//     isLngLatFormat: true,
...
//     //? previos orbits 
//     prev = threeOrbitsArr[0]
//     prev.forEach(() => {
//         creatPoint('r', prev[1], prev[0])
//         console.log(true)
//     })
// });
...
function getISS() {
    ...
    model.position.set(calcPosFromLatLonRad(latLonISS.lat, latLonISS.lng, 16)[0], calcPosFromLatLonRad(latLonISS.lat, latLonISS.lng, 16)[1], calcPosFromLatLonRad(latLonISS.lat, latLonISS.lng, 16)[2])
...
StartTraking.addEventListener('click' ,() => {
    console.log('clicked')
    setInterval(() => {
    const traceGeo = new THREE.SphereGeometry(.01, 30, 30)
    ...
    latLonISS = getLatLngObj(ISS_TLE);
    earth.add(trace)
    trace.position.set(calcPosFromLatLonRad(latLonISS.lat, latLonISS.lng, 16)[0], calcPosFromLatLonRad(latLonISS.lat, latLonISS.lng, 16)[1], calcPosFromLatLonRad(latLonISS.lat, latLonISS.lng, 16)[2])
}, 5000)
